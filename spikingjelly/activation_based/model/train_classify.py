...
modification history:
    ...
    [Fri 11, Jul 2025] sczya: the results for ImageNet1k can not reach the same as the paper, consider to change lr_scheduler
    added <eta_min> for CosineAnnealingLR, which is the minimum learning rate at the end of the cosine annealing schedule.

    [Fri 11, Jul 2025] sczya: changed the creation of skfold_indices
    it assume the indices for one dataset only created at the first time, and all future experiments will use the same indices
    obviously, we can not garantee the quality of the division, so consider all 5 folds results

hyperparameters:

    depends on the datasets and tasks:
...
class Trainer:
    ...
    def evaluate(self, args, model, criterion, data_loader, device, log_suffix=""):
        model.eval()
        metric_logger = utils.MetricLogger(delimiter="  ")
        header = f"{datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')} Test: {log_suffix}"
        ...
        print(f'{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} Test: test_acc1={test_acc1:.3f}, test_acc5={test_acc5:.3f}, test_loss={test_loss:.6f}, samples/s={num_processed_samples / (time.time() - start_time):.2f}')
    ...
    def load_CIFAR10(self, args):
        ...
        dataset = torchvision.datasets.CIFAR10(
            root='/local/data/acxyle/Datasets/cifar-10-batches-py',
            train=True,
            download=True,
    ...
    def load_fused_data(self, args, ):
        """ 
        if does not detected sk list file, generate it
        if detected, load it
        
        make sure all models use the same division of the dataset, move the generated skfold_indeces to <Datasets> then 
        other training script use it.
        
        be careful, this code hopes to divide the dataset only one time, and use the same indices for all experiments
        """
        ...
        if utils.is_main_process():
            ...
            if not os.path.exists(skfold_indices_path_local):
                if os.path.exists(skfold_indices_path_datasets):
                    ...
                    shutil.copy2(skfold_indices_path_datasets, skfold_indices_path_local)
                else:
                    print(f"Main process: Generating skfold indices and saving to {skfold_indices_path_local}")
                    ...
                    with open(skfold_indices_path_local, "w") as f:
                        json.dump(_skfold_indices_list, f)
        ...
        with open(skfold_indices_path_local, "r") as f:
            skfold_indices_list = json.load(f)
    ...
    def set_lr_scheduler(self, args, optimizer):
        ...
        if args.lr_scheduler == "step":
            ...
            main_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs-args.lr_warmup_epochs, eta_min=args.lr*args.lr_warmup_decay)
    ...
    def main(self, args):
        ...
        # ---
        if args.data_fold_training:
            args.output_dir = os.path.join(args.output_dir, f'runs/{args.data_fold_index}')
            utils.mkdir(args.output_dir)
            with open(os.path.join(args.output_dir, "README - datafold training activated.txt"), "w", encoding="utf-8") as f:
                f.writelines([
                                "this training is based on dataset-level fold training \n",
                                f"the total number of folds is [{args.data_fold_number}], the current index is [{args.data_fold_index}]\n",
                                "please check skfold_indices.json to check the datset division\n"
                            ])
        else:
            with open(os.path.join(args.output_dir, "README - random division activated.txt"), "w", encoding="utf-8") as f:
                f.writelines([
                                "this training is based on the random division of train/val datasets\n",
                                "please check the first group of skfold_indices.json to check the datset indices\n",
                            ])
        ...
        if utils.is_main_process():
            os.makedirs(tb_dir, exist_ok=args.resume is not None)     # this design does not allow 'identical' training
            os.makedirs(pt_dir, exist_ok=args.resume is not None)

        if args.resume is not None:
            if args.resume == 'latest':
                checkpoint = torch.load(os.path.join(pt_dir, 'checkpoint_latest.pth'), weights_only=False, map_location="cpu")
            else:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if utils.is_main_process():
                ...
                if utils.is_main_process() and epoch > 0:
                    previous_check_point = os.path.join(pt_dir, f"checkpoint_{epoch - 1}.pth")
                    if os.path.exists(previous_check_point):
                        os.remove(previous_check_point)
                    
            print(f'{datetime.datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")} escape time={(datetime.datetime.now() + datetime.timedelta(seconds=(time.time() - start_time) * (args.epochs - epoch))).strftime("%Y-%m-%d %H:%M:%S")}\n')
